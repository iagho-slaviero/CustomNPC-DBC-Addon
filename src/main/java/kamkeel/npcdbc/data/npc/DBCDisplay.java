...
public class DBCDisplay implements IDBCDisplay, IAuraData {
    ...
    public void setRacialExtras() {
        ...
        if (kamkeel.npcdbc.constants.DBCRace.isSaiyan(this.race)) {
        ...
        } else if (this.race == kamkeel.npcdbc.constants.DBCRace.ARCOSIAN) {
            noppes.npcs.entity.data.ModelPartData tail = data.getOrCreatePart("tail");
            tail.setTexture("tail/monkey1", 8);
            tail.pattern = 2;
            ...
            if (arcoState != 4 || hasCustomHorns) {
                ...
                if (arcoState == 3) {
                    ...
                    data.getOrCreatePart("dbcArms").setTexture("tail/monkey1", 2);
                }
                if (arcoState == 5) {
                    horn.setTexture("tail/monkey1", 5);
                    data.getOrCreatePart("dbcArms").setTexture("tail/monkey1", 1);
                    data.getOrCreatePart("dbcBody").setTexture("tail/monkey1", 1);
                }
            }
